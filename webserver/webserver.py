...
class WebServer(commands.Cog):
    ...
    @webserver_group.command(name="set")
    ...
    async def set_port(self, ctx, port: int):
        ...
        await ctx.send(f"Web server port set to {port}. Restart the cog for changes to take effect.")

    @webserver_set.command(name="host")
    async def set_host(self, ctx, host: str):
        """Set the host for the web server."""
        await self.config.host.set(host)
        await ctx.send(f"Web server host set to {host}. Restart the cog for changes to take effect.")
    ...
    async def show_config(self, ctx):
        ...
        # Send the config via DM to keep the API key private
        try:
            ...
            # If DMs are disabled, show a redacted version
            await ctx.send(
                f"**Web Server Configuration**\n"
                f"Host: `{host}`\n"
                f"Port: `{port}`\n"
