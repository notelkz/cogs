...
class AppTestView(View):
    def __init__(self, cog, guild: discord.Guild, user: discord.Member, embed: discord.Embed, thumbnail_file: Optional[discord.File]):
...
class AppTrack(commands.Cog):
    ...
    async def discover_activities(self, ctx: commands.Context):
        ...
        for activity_name, first_seen in sorted_activities:
            ...
            try:
                ...
                first_seen_str = first_seen_dt.strftime("%Y-%m-%d %H:%S")
            except:
    ...
    async def apptest(self, ctx: commands.Context, channel_mention: str = None):
        ...
        view = AppTestView(self, ctx.guild, ctx.author, embed, thumbnail_file)
