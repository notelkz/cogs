...
class CalendarSyncLogic:
    ...
    def __init__(self, cog_instance):
        ...
        # Initialize the loop directly here
        self.sync_loop_task = self.sync_events_periodic # Reference the decorated method

    def start_tasks(self):
        """Starts the periodic event sync task."""
        # Check if the task is already running
        if self.sync_loop_task.is_running():
            log.info("CalendarSync: Task is already running, skipping start.")
            return

        # Fetch interval from config before starting the task
        self.cog.bot.loop.create_task(self._start_sync_loop_wrapper())

    async def _start_sync_loop_wrapper(self):
        await self.cog.bot.wait_until_ready()
        # Use cal_interval if available, otherwise default to 15
        interval_minutes = await self.config.cal_interval() if hasattr(self.config, 'cal_interval') else 15
        
        if interval_minutes < 1:
            log.warning("CalendarSync: Interval is less than 1 minute, defaulting to 1 minute.")
            interval_minutes = 1
            
        self.sync_loop_task.change_interval(minutes=interval_minutes)
    ...
    def stop_tasks(self):
        ...
        if self.sync_loop_task and self.sync_loop_task.is_running():
            self.sync_loop_task.cancel()
            log.info("CalendarSync: sync loop cancelled.")
        else:
            log.info("CalendarSync: sync loop not running or not initialized.")
    
    @tasks.loop(minutes=15) # Actual interval set by _start_sync_loop_wrapper
            ...
            try:
                ...
                for event in events:
                    ...
                    if not exists:
                        ...
                        log.info(f"CalendarSync: Discord event '{event.name}' already exists on website. Skipping.")
            except Exception as e:
    ...
    async def create_discord_event(self, event_data: Dict[str, Any]) -> Optional[discord.ScheduledEvent]:
        ...
        guild_id_env_str = os.environ.get("DISCORD_GUILD_ID") # Re-using this env var as ActivityTracker did
        guild = self.cog.bot.get_guild(int(guild_id_env_str)) if guild_id_env_str and guild_id_env_str.isdigit() else self.cog.bot.guilds[0] if self.cog.bot.guilds else None
    ...
    async def on_scheduled_event_update(self, before, after):
        ...
        try:
            ...
            api_key = await self.config.cal_api_key()
            if not api_url or not api_key:
                log.warning("CalendarSync: API URL or API key not set. Cannot sync event update.")
                return

            async with self.session.get( # Use central session
                f"{api_url}?discord_event_id={after.id}",
                headers={"Authorization": f"Token {api_key}", "X-API-Key": api_key}
            ) as resp:
                if resp.status != 200:
    ...
    async def on_scheduled_event_delete(self, event):
        ...
        try:
            ...
            api_key = await self.config.cal_api_key()
            if not api_url or not api_key:
                log.warning("CalendarSync: API URL or API key not set. Cannot sync event deletion.")
                return

            async with self.session.get( # Use central session
                f"{api_url}?discord_event_id={event.id}",
                headers={"Authorization": f"Token {api_key}", "X-API-Key": api_key}
            ) as resp:
                if resp.status != 200:
    ...
    async def show_config(self, ctx):
        ...
        try:
            await ctx.author.send(
                ...
                f"Use `{ctx.prefix}zll calendar setapiurl` and `{ctx.prefix}zll calendar setapikey` to update these settings."
            )
