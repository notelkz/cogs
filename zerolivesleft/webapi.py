...
class WebApiManager:
    ...
    def register_all_routes(self):
        ...
        try:
            ...
            # --- User Profile Routes ---
            # General user details (display name, avatar, all roles). Accessible via /api/user/{user_id}
            self.web_app.router.add_get("/api/user/{user_id}", self.get_user_details_handler)

            # Route for getting just user roles. Accessible via /api/user/{user_id}/roles
            self.web_app.router.add_get("/api/user/{user_id}/roles", self.get_user_roles_handler) 
