...
class Zerolivesleft(commands.Cog):
    ...
    # Fixed: Define militaryranks as a group that takes subcommands
    ...
    # Fixed: Properly define subcommands for militaryranks
    @activity_militaryranks_commands.command(name="add")
    async def militaryranks_add(self, ctx, role: discord.Role, required_hours: float):
        """Add a new military rank."""
        await self.activity_tracking_logic.add_rank(ctx, role, required_hours)
    
    @activity_militaryranks_commands.command(name="remove")
    async def militaryranks_remove(self, ctx, role_or_name: str):
        """Remove a military rank by role ID or name."""
        await self.activity_tracking_logic.remove_rank(ctx, role_or_name)
    
    @activity_militaryranks_commands.command(name="clear")
    async def militaryranks_clear(self, ctx):
        """Clear all configured military ranks."""
        await self.activity_tracking_logic.clear_ranks(ctx)
    
    @activity_militaryranks_commands.command(name="list")
    async def militaryranks_list(self, ctx):
